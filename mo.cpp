...
{
    double length = 0; 
    set<vertex*> S; // uintptr_t
    Heap priority_q(MAXSIZE);
    assert(this->source->key == 0);
    priority_q.insert(source);
    cout << "Source is: " << this->source->name << endl;
    while (priority_q.size > 0)
    {
        priority_q.print_tree();
        vertex* v = priority_q.extract_min();
        //....................... THIS DID NOT WORK
        //length += v->key; 
        // ...........................
        cout << "Vertex out is: " << v->name << endl;
        S.insert(v);
        //double l_min = INF; 
        for (auto it = v->adj.begin(); it != v->adj.end(); ++it)
        {
            vertex* w = it->first;
            double l = it->second;
            cout << "W is: " << w->name << endl;
            cout << "length to w is: " << l << endl;
            
            if (S.count(w)) {
                cout << "w in set already" << endl;
                continue;
            }
            cout << "w->key is: " << w->key << endl;
            cout << "l is: " << l << endl;
            cout << "w->key > l:" << (w->key > l) << endl;
            if (w->key > l) 
            {
                cout << "Updating w" << endl;
                w->key = l;
                // TODO: Update prev[] if we get to add this feature
                //length += l;
                priority_q.insert(w);
                cout << "Value of length after update:" << length << endl;
                cout << "Printing tree after update of w" << endl;
...
int main() 
{
    ...
    vector<double> stats_v;
    size_t stats_v_count = 0;
  // Add edges
    for (auto a= myGraph.vertices.begin(); a != myGraph.vertices.end(); a++)
    {
        ...
        for(auto b = myGraph.vertices.begin(); b != myGraph.vertices.end(); b++)
        {
            ...
            double l = sqrt(pow((b->x - a->x), 2) + pow((b->y - a->y), 2) + pow((b->z - a->z), 2) + pow((b->w - a->w), 2));
            myGraph.draw_edge(&(*a), &(*b), l);
            cout << "New Edge (" << a->name << ", " << b->name << ", length = " << l << ") " << endl;
            stats_v.push_back(l);
        }
    }

    vector<double> all_possible_sums;

    for (size_t i = 0; i < stats_v.size()-2; i++)
    {
        for (size_t j = i+1; j < stats_v.size()-1; j++)
        {
            for (size_t k = j+1; k < stats_v.size(); k++)
            {  
                cout << i << " " << j << " " << k << endl;
                all_possible_sums.push_back(stats_v[i]+ stats_v[j] + stats_v[k]);
            }
        }
    }
    
    for(auto it = all_possible_sums.begin(); it != all_possible_sums.end(); it++)
    {
        cout << *it << endl;
    }

    cout << "MINIMUM IS: " << *min_element(all_possible_sums.begin(), all_possible_sums.end()) << endl;
